...
from instructions import txt_instruction, txt_test1, txt_test2, txt_test3, txt_sits, txt_taco, txt_burito, txt_test4
from ruffier import test

from seconds import Seconds
from sits import Sits, Sats
from runner import Runner, Ruiner
   ...
   def next(self):
       if not self.next_screen:
           ...
           if p1 == False or p1 <= 0:
               ...
               self.manager.current = 'taco'

class CheckTaco(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.next_screen = False

        instr = Label(text=txt_taco, size_hint=(0.5, 1))
        self.lbl_sats = Sats(10)
        self.run = Ruiner(total=10, steptime=1.5, size_hint=(0.4, 1))
        self.run.bind(finished=self.run_finished)

        line = BoxLayout()
        vlay = BoxLayout(orientation='vertical', size_hint=(0.3, 1))
        vlay.add_widget(self.lbl_sats)
        line.add_widget(instr)
        line.add_widget(vlay)
        line.add_widget(self.run)
    ...
    def next(self):
        if not self.next_screen:
            self.btn.set_disabled(False)
            self.run.start()
            self.run.bind(value=self.lbl_sats.next)
        else:
            self.manager.current = 'pulse3'

class PulseScr32(Screen):
   def __init__(self, **kwargs):
       super().__init__(**kwargs)
       self.next_screen = False
   ...
   def next(self):
       if not self.next_screen:
           ...
           if p1 == False or p1 <= 0:
               ...
               self.manager.current = 'burito'



class CheckBurito(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.next_screen = False

        instr = Label(text=txt_burito, size_hint=(0.5, 1))
        self.lbl_sits = Sits(20)
        self.run = Runner(total=20, steptime=1.5, size_hint=(0.4, 1))
        self.run.bind(finished=self.run_finished)
...
class HeartCheck(App):
   def build(self):
       ...
       sm.add_widget(PulseScr32(name='pulse3'))
       sm.add_widget(CheckSits(name='sits'))
       sm.add_widget(Pulse4(name='pulse4'))
       sm.add_widget(CheckTaco(name='taco'))
       sm.add_widget(CheckBurito(name='burito'))
       sm.add_widget(PulseScr2(name='pulse2'))
       sm.add_widget(Result(name='result'))
       img = Image(source='gachi.jpg',
                    size_hint=(1, .5),
                    pos_hint={'center_x':.5, 'center_y':.5}) 
       return sm
